...
def parse_args():
	...
	parser.add_argument("--base_model", default="Qwen/Qwen1.5-14B", type=str)
	parser.add_argument("--sub_model", default="Qwen/Qwen1.5-7B", type=str)
	parser.add_argument("--add_model", default="Qwen/Qwen1.5-7B", type=str)
...
def setup(args):
	...
	for prompt in prompt_pairs:
		if "-with-demos" in prompt:
			print("==" * 10)
			print("With demos")
			prompt = prompt.replace("-with-demos", "")
			samples, _, _ = model_diff.create_response_prompt_and_format(
				args, args.data_name, prompt, reverse=True, num_shots=-1
			)
		else:
			samples, _, _ = model_diff.create_response_prompt_and_format(
	...
	model_diff.generate_all_vectors(args, dataset_prompts_pair, args.hs_diff_save_path)
