...
def parse_args():
	...
	parser.add_argument("--layer_to_add", default="0", type=str)
...
def setup(args):
	...
	print(args.hs_diff_save_path)
	reasoning_vector = torch.load(args.hs_diff_save_path, map_location="cpu")
	reasoning_vector = torch.FloatTensor(reasoning_vector)

	model_diff.load_model_tokenizer(
		args.base_model_name_or_path,
		reasoning_vector=reasoning_vector,
		transfer=True,
		layer_to_add=args.layer_to_add,
		alpha=args.alpha,
	)
	...
	for data_name, prompt_type in zip(data_list, prompt_list):
		out_file = create_filename(args, data_name, prompt_type)
...
def create_filename(args, data_name, prompt_type):
	...
	out_file_prefix += f"layer_{args.layer_to_add}_alpha_{args.alpha}_{args.split}_num_samples_{args.num_test_sample}_max_gen_tokens_{args.max_new_tokens}"
