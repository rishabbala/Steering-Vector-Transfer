...
ranks = [1, 4, 16, 64, 128, 256, 512]
markers = ["o", "X", "s", "^", "D", "v", "P", "H"]
cmap = plt.get_cmap("viridis")
n_ranks_list = len(ranks)
t = np.linspace(0.1, 0.95, n_ranks_list)
...
def compute_l2_error(student, teacher, transform=None):
	...
	error = torch.norm(teacher - student_transformed, p=2) / torch.norm(teacher, p=2)
	return error.item()
...
def main():
	...
	for base_model, sub_model in MODEL_PAIRS:
		OUTPUT_FILE = f"./figures/transform_error_rank/transformation_error_rank_{num_examples}_{base_model.split('/')[-1]}_sub_{sub_model.split('/')[-1]}_data_gsm8k_math.pdf"
		...
		TRANSFORM_FILES = {
			data: [
				f"/projects/llms-lab/transfer_compare/spectral_entropy/num_train_samples_{num_examples}/base_{base_model.split('/')[-1]}_sub_{sub_model.split('/')[-1]}_pca_data_{data}_rank_1_num_train_samples_{num_examples}.pth",
				f"/projects/llms-lab/transfer_compare/spectral_entropy/num_train_samples_{num_examples}/base_{base_model.split('/')[-1]}_sub_{sub_model.split('/')[-1]}_pca_data_{data}_rank_4_num_train_samples_{num_examples}.pth",
				f"/projects/llms-lab/transfer_compare/spectral_entropy/num_train_samples_{num_examples}/base_{base_model.split('/')[-1]}_sub_{sub_model.split('/')[-1]}_pca_data_{data}_rank_16_num_train_samples_{num_examples}.pth",
				f"/projects/llms-lab/transfer_compare/spectral_entropy/num_train_samples_{num_examples}/base_{base_model.split('/')[-1]}_sub_{sub_model.split('/')[-1]}_pca_data_{data}_rank_64_num_train_samples_{num_examples}.pth",
				f"/projects/llms-lab/transfer_compare/spectral_entropy/num_train_samples_{num_examples}/base_{base_model.split('/')[-1]}_sub_{sub_model.split('/')[-1]}_pca_data_{data}_rank_128_num_train_samples_{num_examples}.pth",
				f"/projects/llms-lab/transfer_compare/spectral_entropy/num_train_samples_{num_examples}/base_{base_model.split('/')[-1]}_sub_{sub_model.split('/')[-1]}_pca_data_{data}_rank_256_num_train_samples_{num_examples}.pth",
				f"/projects/llms-lab/transfer_compare/spectral_entropy/num_train_samples_{num_examples}/base_{base_model.split('/')[-1]}_sub_{sub_model.split('/')[-1]}_pca_data_{data}_rank_512_num_train_samples_{num_examples}.pth",
			]
			for data in DATA_NAMES
		}
		...
		for dataset_idx, data_name in enumerate(DATA_NAMES):
			...
			for idx, transform_file in enumerate(TRANSFORM_FILES[data_name]):
				...
				rank_str = transform_file.split("rank_")[-1].split(
					"_num_train_samples_"
				)[0]
				rank = int(rank_str)
				...
				label = r"$\mathrm{k}$" + f"={rank}" if rank else f"Transform {idx + 1}"
			...
			leg = ax.legend(loc="upper left", title="Rank", title_fontsize=20)
