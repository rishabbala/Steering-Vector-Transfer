...
OOD_AVG_DIR = BASE_DIR / "hs_svd_arch_test_transfer_post_training_OOD_avg"
OOD_PCA_DIR = BASE_DIR / "hs_svd_arch_test_transfer_post_training_OOD_pca"
BASE_EVAL_DIR = BASE_DIR / "base_evals_test"

DATASETS: List[str] = ["agieval_math", "minerva_math", "olympiadbench", "deepmind_math"]
FIGURES_DIR = Path("/home/rishbb/Qwen2.5-Math/figures")
MODEL = "Qwen3-14B-Base_+_Qwen3-4B_-_Qwen3-4B-Base"
OUTPUT_PATH = FIGURES_DIR / "plot_math.pdf"

PAPER_NAME = {
	"Qwen3-14B-Base_+_Qwen3-4B_-_Qwen3-4B-Base": r"Qwen3-4B $\rightarrow$ Qwen3-14B",
}

PAPER_DATASET_NAME = {
	"agieval_math": "AGIEval\nMath",
	"minerva_math": "Minerva\nMath",
	"olympiadbench": "Olympiad\nBench",
	"deepmind_math": "Deepmind\nMath",
}
...
def get_instruct_model_name(base_model: str) -> str:
	if "Qwen3" in base_model:
		return base_model.replace("-Base", "")
	return base_model
...
def read_cot_acc_for_model_family(model_family: str, dataset: str) -> Optional[float]:
	...
	mp = _find_baseline_cot_metrics(base_model, dataset)
	if mp is None:
		return None
	return _read_acc(mp)
...
def read_instruct_cot_acc_for_model_family(
	...
	mp = _find_baseline_cot_metrics(instruct_model, dataset)
	if mp is None:
		return None
	return _read_acc(mp)
...
def plot_accuracy(best_transfer: Dict[Tuple[str, str], TransferBest]) -> None:
	...
	for dataset in DATASETS:
		dacc = read_cot_acc_for_model_family(base_model_name, dataset)
	...
	ax.bar(
		...
		best_vals,
		bar_width,
		label=f"Qwen3-14B-Base + {to_small_caps('unlock')}$_{{ \mathrm{{from\;4B}}}}$",
