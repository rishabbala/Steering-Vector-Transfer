...
import numpy as np
from grader import *
from ifeval import process_results
from parser import *
...
def evaluate(
	...
	if data_name == "pop_qa":
		...
		acc = 0
		for sample in samples:
			if sample["answer"].strip(" ().") == sample["code"][0].strip(" ()."):
				acc += 1
		...
		for sample in samples:
			result = process_results(sample, sample["code"])
			avg_result["prompt_level_strict_acc"] += result[
				"prompt_level_strict_acc"
			] / len(samples)
			avg_result["prompt_level_loose_acc"] += result[
				"prompt_level_loose_acc"
			] / len(samples)
			avg_result["inst_level_strict_acc"] += (
				sum(r for r in result["inst_level_strict_acc"])
				/ len(result["inst_level_strict_acc"])
			) / len(samples)
			avg_result["inst_level_loose_acc"] += (
				sum(r for r in result["inst_level_loose_acc"])
				/ len(result["inst_level_loose_acc"])
			) / len(samples)
		return samples, {
			"num_samples": len(samples),
			"acc": avg_result["prompt_level_strict_acc"] * 100,
			"prompt_level_strict_acc": avg_result["prompt_level_strict_acc"] * 100,
			"prompt_level_loose_acc": avg_result["prompt_level_loose_acc"] * 100,
			"inst_level_strict_acc": avg_result["inst_level_strict_acc"] * 100,
			"inst_level_loose_acc": avg_result["inst_level_loose_acc"] * 100,
		}
		...
		for k in k_values:
			...
			pass_at_k[k] = float(
				np.round(np.mean(pass_at_k_estimates) * 100, decimals=1)
			)
