...
class Gemma3RMSNorm(nn.Module):
	...
	def forward(self, x):
		...
		# Llama does x.to(bfloat16) * w whilst Gemma3 is (x * w).to(bfloat16)
	...
	def forward(
		...
		**kwargs: Unpack[TransformersKwargs],
		...
		for didx, decoder_layer in enumerate(
			...
			if self.transfer and hidden_states.shape[1] != 1:
				rvector = (
				...
				hnorm = torch.norm(last_hidden, dim=-1, keepdim=True)
				rnorm = torch.norm(rvector, dim=-1, keepdim=True)

				# Normalize and add steering vector
				normalized_hidden = last_hidden / hnorm
				normalized_rvector = rvector / rnorm
				modified_hidden = normalized_hidden + normalized_rvector * self.alpha

				# Renormalize and scale back
				modified_hidden = (
					modified_hidden
					/ torch.norm(modified_hidden, dim=-1, keepdim=True)
					* hnorm
				)

				# Assign back (no longer in-place on original tensor)
				hidden_states[:, -1, :] = modified_hidden
