...
class Olmo3RotaryEmbedding(Olmo2RotaryEmbedding):
	def __init__(
		self, config: Olmo3Config, device=None, rope_type: Optional[str] = None
	):
...
class Olmo3Model(Olmo2Model):
	def __init__(self, config: Olmo3Config):
		...
		self.layers = nn.ModuleList(
			[
				Olmo3DecoderLayer(config, layer_idx)
	...
	def forward(
		...
		**kwargs: Unpack[TransformersKwargs],
		...
		position_embeddings_mapping = {
			"sliding_attention": self.rotary_embs["sliding_attention"](
				hidden_states, position_ids
			),
			"full_attention": self.rotary_embs["full_attention"](
				hidden_states, position_ids
			),
		}
