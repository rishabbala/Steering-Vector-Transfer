...
def parse_args():
	...
	parser.add_argument("--alpha_w", default=1.0, type=float)
	parser.add_argument("--use_params", default=None, type=str)
	parser.add_argument("--direction", default="fixed", type=str)
...
def setup(args):
	...
	model_diff.generate_weight_diff_sizes(args)
	...
	for data_name in data_list:
		...
		if out_file is None:
			continue
		results.append(model_diff.generate_response(args, data_name, out_file))
...
def create_filename(args, data_name):
	# get out_file name
	model_name = "/".join(args.student_model_name_or_path.split("/")[-2:]).replace(
		"/", "_"
	)
	out_file_prefix = f"{model_name}_"
	out_file_prefix += f"weight_diff_direction_{args.direction}_"
	out_file_prefix += f"{args.split}_{args.prompt_type}_{args.num_test_sample}_seed{args.seed}_t{args.temperature}_alpha_{args.alpha_w}_use_params_{args.use_params}"
