...
USE_PARAMS=(\
	...
	# "lm_head,input_layernorm,up_proj,gate_proj,down_proj,norm_weight" \
	# "lm_head,up_proj,gate_proj,down_proj,norm_weight" \
	# "lm_head,up_proj,gate_proj,down_proj" \
	# "embed,up_proj,gate_proj,down_proj"
	# "embed,up_proj,gate_proj,down_proj,lm_head"
)

OUTPUT_ROOT="/projects/llms-lab/simple_rl_math_eval_arch"
SPLIT="test"
NUM_TEST_SAMPLE=100
	...
	for direction in "${!DIRECTION[@]}"; do
		...
		for alpha in "${!ALPHAS[@]}"; do
			...
			for i in "${!BASE_MODELS[@]}"; do
				...
				echo "=== RUNNING weight transfer ${base}, ${student}, ${teacher}, ${prompt}, ${alpha}, ${direction}, ${use_param} ==="
				for j in "${!DATA_TYPES[@]}"; do
					data="${DATA_TYPES[$j]}"
					task="${TASK_TYPES[$j]}"
					outdir="${OUTPUT_ROOT}/${base##*/}/${task}"
					mkdir -p "${outdir}/logs"
					weight_diff_save_path="${OUTPUT_ROOT}/${student##*/}/${task}/weight_diff_${prompt}_teacher_${teacher##*/}_base_${base##*/}.pth"
